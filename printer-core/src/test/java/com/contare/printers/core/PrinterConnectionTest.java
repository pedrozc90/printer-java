...
public class PrinterConnectionTest {
    ...
    void testReadSingleMessage() throws Exception {
        ...
        final byte[] message = concat((byte) STX, payload.getBytes(charset), (byte) ETX);
    ...
    void testReadMultipleMessagesAndIgnoreLeadingBytes() throws Exception {
        ...
        final byte[] m1 = concat((byte) STX, "ONE".getBytes(charset), (byte) ETX);
        final byte[] m2 = concat((byte) STX, "TWO".getBytes(charset), (byte) ETX);
        ...
        try {
            ...
            assertArrayEquals(m1, results.get(0).getBytes(charset));
            assertArrayEquals(m2, results.get(1).getBytes(charset));
        } finally {
            pc.close();
        }
